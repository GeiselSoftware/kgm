...
class UserObjectMemberEditor:
    ...
    def create_RDFObject__(v:object) -> RDFObject:
        if isinstance(v, UserObject):
            ret = RDFObject(v.get_impl().get_uri())
        else:
    ...
    def get_dels_inss__(self): # returns (set<RDFTriple>, set<RDFTriple>)
        ...
        dels = {RDFTriple(self.uo.get_impl().get_uri(), self.m_path_uri, self.create_RDFObject__(x)) for x in dels_T}
        inss = {RDFTriple(self.uo.get_impl().get_uri(), self.m_path_uri, self.create_RDFObject__(x)) for x in inss_T}
        return (dels, inss)

...
shared_ptr<VisNode> RDFNodeManager::create_vis_node(RDFNode* n)
{
	...
	if (spo.p.uri == "kgm:node_vis_color") {
...
void RDFNodeManager::start_load_graph(const string& kgm_path, const string& fuseki_server_url)
{
  ...
  string rq = fmt::format(rq_fmt, kgm_path);

...
class Database:
    def __init__(self, fuseki_url, kgm_path):
    ...
    def create(self, uc_uri:URI) -> UserObject:
    ...
    def get_dels_inss__(self):
        ...
        for uo in self.just_created_uo:
            inss.append(RDFTriple(uo.get_impl().get_uri(), rdf.type, uo.get_impl().uc_uri))
    ...
    def add_user_object(self, uo: UserObject, is_just_created: bool):
        self.all_user_objects[uo.get_uri()] = uo
        if is_just_created:
            self.just_created_uo.add(uo)

    def get_user_object(self, uri:URI) -> UserObject:
    ...
    def load(self, uo_uri: URI) -> UserObject:
        ipdb.set_trace()
